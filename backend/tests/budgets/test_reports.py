...
class TestReports:
    def test_monthly_summary(self, auth_client, seeded_space):
        ...
        response = auth_client.get(f"/api/budgets/reports/monthly-summary/?space_id={space_id}&month=2026-03")
    ...
    def test_weekly_summary(self, auth_client, seeded_space):
        ...
        space_id, cat = seeded_space
        response = auth_client.get(f"/api/budgets/reports/weekly-summary/?space_id={space_id}&week=2026-03-01")
        assert response.status_code == 200
        totals = {r["category_id"]: r["total"] for r in response.data}
    ...
    def test_yearly_summary(self, auth_client, seeded_space):
        ...
        space_id, cat = seeded_space
        response = auth_client.get(f"/api/budgets/reports/yearly-summary/?space_id={space_id}&year=2026")
        assert response.status_code == 200
        totals = {r["category_id"]: r["total"] for r in response.data}
    ...
    def test_monthly_bad_format_returns_400(self, auth_client, seeded_space):
        ...
        response = auth_client.get(f"/api/budgets/reports/monthly-summary/?space_id={space_id}&month=not-a-date")
        assert response.status_code == 400

    def test_weekly_bad_format_returns_400(self, auth_client, seeded_space):
        """A malformed week parameter returns 400."""
        space_id, _ = seeded_space
        response = auth_client.get(f"/api/budgets/reports/weekly-summary/?space_id={space_id}&week=not-a-date")
        assert response.status_code == 400

    def test_yearly_bad_format_returns_400(self, auth_client, seeded_space):
        """A malformed year parameter returns 400."""
        space_id, _ = seeded_space
        response = auth_client.get(f"/api/budgets/reports/yearly-summary/?space_id={space_id}&year=not-a-number")
        assert response.status_code == 400

    def test_unknown_report_type_returns_404(self, auth_client, seeded_space):
        """Requesting a non-existent report type returns 404."""
        space_id, _ = seeded_space
        response = auth_client.get(f"/api/budgets/reports/nonexistent/?space_id={space_id}&month=2026-03")
